...
#########
def transform_template(input_text):
    # Parse the input text using the defined grammar
    parsed_result = grammar.parseString(input_text)

    # Initialize transformed segments
    user_segments = []
    assistant_segments = []

    # Helper function to create tags
    def create_tag(segment, tag):
        return "{{{}}}{}".format(tag, segment)

    # Process parsed result
    for chunk in parsed_result.program:
        if chunk.name == 'user':
            user_segments.append(chunk.text)
        elif chunk.name == 'assistant':
            assistant_segments.append(chunk.text)
        else:
            user_segments.append(create_tag(chunk.text, "#user"))
            assistant_segments.append(create_tag("", "#assistant"))

    # Ensure the template ends with {{gen 'write'}}
    if not any(chunk for chunk in assistant_segments if "gen 'write'" in chunk):
        assistant_segments.append("{{gen 'write' }}")

    # Construct the final transformed template
    user_text = ' '.join(user_segments)
    assistant_text = ' '.join(assistant_segments)
...
output1_user, output1_assistant = transform_template(input1)
output2_user, output2_assistant = transform_template(input2)

print("Input 1:")
print("User:", output1_user)
print("Assistant:", output1_assistant)
print()

print("Input 2:")
print("User:", output2_user)
print("Assistant:", output2_assistant)
